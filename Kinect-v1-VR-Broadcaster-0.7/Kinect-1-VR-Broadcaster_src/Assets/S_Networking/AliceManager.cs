...
public class AliceManager : MonoBehaviour {
	...
	public Alice net;
	public string host = "192.168.0.86";
	...
	void Start () {
		...
		//ConnectServer ();
		string savedhost = PlayerPrefs.GetString("host");


		if (savedhost != "") {
			host = savedhost;
		} else {
			host = Network.player.ipAddress;
		}
		...
		*/
		...
		foo = new Process();
		foo.StartInfo.FileName = Application.dataPath + "/KinectVRServer/KinectVR-Server-Launcher.bat";
	...
	*/
	...
	void StartServer() {
		if (host != Network.player.ipAddress) {
		...
		//BodySourceManager.s.StartTracking ();
			...
			Login (name);
			/*
			AliceSync.s.CreateBody (0);
			AliceSync.s.CreateBody (1);
			*/
	...
	void OnGUI () {
		...
		//name = GUI.TextField (new Rect (10, 70, 200, 20), name, 25);
			/*
			if (GUI.Button(new Rect(10, 140, 100, 30), "Connect"))
				ConnectServer ();
			*/
